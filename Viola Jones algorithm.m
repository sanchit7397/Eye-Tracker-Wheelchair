    ...
    img = flip(vid, 2); % Flips the image horizontally
     ...
     if ~ isempty(bbox)  %if face exists 
         ...
         if ~ isempty(bboxeyes)  %check it eyepair is available
             ...
             bboxeyeshalf=[bboxeyes(biggest_box_eyes,1),bboxeyes(biggest_box_eyes,2),bboxeyes(biggest_box_eyes,3)/2.2,bboxeyes(biggest_box_eyes,4)];   %resize the eyepair width in half
